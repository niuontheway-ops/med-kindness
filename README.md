...
- 入口文件：`desktop.html`
- 单文件离线版：`downloads/有温度的问诊室-电脑离线版.html`
- 保留三栏沉浸式舞台、病例进度、关键信息和能力仪表。
- 顶部“手机扫码版”按钮可随时显示二维码，在电脑与手机间切换。

下载后直接双击这个 HTML 文件即可。病例、人物、场景、样式、对话和评分全部内嵌在同一个文件中，不需要解压、BAT、Node.js 或本地服务器，核心学习流程不需要联网。
...
- 舞台会依据人物原生朝向自动交换医患左右位置，保证双方保持面对面；医生还会随回合在主动倾听、共情承接、解释与共决、急症评估、温和设限五种姿态间切换。
- 选择后患者或家属会在悲伤、害怕、愤怒加剧与释然之间切换。
...
人物、场景图、15 张患者/家属情绪变体及 4 张新增医生姿态均为项目生成式原创素材。本应用无后端，不收集或上传学习数据；成绩仅保存在浏览器 `localStorage` 中。
